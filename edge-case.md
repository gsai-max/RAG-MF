...
* **Vercel Build Error with PostCSS Config Dynamic Imports**:
  * *Description:* Building the frontend on Vercel under certain Node versions (such as Node 22 or 24) throws `SyntaxError: Invalid regular expression: missing /` when the dynamic module loader cache-buster query (`?t=...`) inside `postcss-load-config` attempts to resolve `postcss.config.js`.
  * *Mitigation:* Configure PostCSS plugins (Tailwind CSS and Autoprefixer) inline directly inside [vite.config.ts](file:///c:/Nextleap%20Projects%20Git/RAGMF/frontend/vite.config.ts) and remove the external `postcss.config.js` file entirely. This prevents `postcss-load-config` from executing and avoids the dynamic import regex parsing error completely.
