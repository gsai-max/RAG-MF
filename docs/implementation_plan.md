...
* **Tasks**:
  ...
  2. Implement index creator ([index.py](file:///c:/Nextleap%20Projects%20Git/RAGMF/src/app/ingestion/index.py)) using ChromaDB, configured to dynamically load the `SentenceTransformer` (BGE-small) model locally in dev/ingestion, but route through the Hugging Face Inference API in production to save RAM (bypassing local PyTorch imports on environments like Render Free Tier). The custom `HuggingFaceInferenceEmbeddingFunction` class implements `name() -> "sentence_transformer"` to satisfy ChromaDB protocols and ensure compatibility with indices generated across environments.
...
* **Tasks**:
  ...
  3. Configure a scheduled GitHub Actions workflow ([daily-scheduler.yml](file:///c:/Nextleap%20Projects%20Git/RAGMF/.github/workflows/daily-scheduler.yml)) to run `scheduler/daily.py --now` daily at 9:15 AM IST (03:45 UTC), automatically on push to `main`, or via manual triggering, utilizing `--fast` execution delay (`0.1s - 0.3s`), dependency caching (raw scraper dumps & HuggingFace hub models), and zip artifact archiving. (Completed)
* **Deliverables**: Daily crawler scheduler script, atomic swap workflow logs, and optimized GitHub Actions workflow file with build run database artifacts.
...
* **Tasks**:
  ...
  5. Configure standard, extensionless imports (e.g. `import App from './App'`), remove `allowImportingTsExtensions` from `tsconfig.json`, and update the package build script to `vite build` to bypass separate type-checking blocks on Vercel's build container. Also configured PostCSS plugins (Tailwind CSS) inline directly in [vite.config.ts](file:///c:/Nextleap%20Projects%20Git/RAGMF/frontend/vite.config.ts), removed the external `postcss.config.js` file entirely, and removed `autoprefixer` and `postcss` devDependencies to bypass Vercel's dynamic dependency analysis (`@vercel/nft`) regex parsing bug, which caused a runtime `SyntaxError: Invalid regular expression: missing /` when loading `browserslist` (a deep dependency of `autoprefixer`) in Vercel environments. (Completed)
